...
%[filename, pathname]= uigetfile({'*.tif','Image Files(*.tif)'},'Choose');
 ...
 pathname = 'G:\Shared drives\Lim_Lab\Sahla\snail enhancer\Imaging\snaSE binding mutations\snaSE_dl1del_M5_2022_09_27_higherpower\nc13\emb04\';
 filename = 'SE_dl1del4_NC13.tif';
% 
fname = [pathname filename];
info = imfinfo(fname);
num_images = numel(info);

export = [pathname 'segmentation_nc13\'];
export2 = pathname;
 
zero = 21; % first frame of nuclear cycle 14
start = 9; % total number of frames in the movie
finish = 57; % frame index where nuclei signal's not accumulated
...
numPass = 1; % 1 = first pass/final pass, 2 = second pass
...
for n=start:finish
    ...
    h = fspecial('gaussian',3,3); % gaussian filter (15 better for NC14/NC15)
%     h = fspecial('gaussian',25,25); % gaussian filter (25 better for
%     NC11/12/13)
...
%     se = strel('disk',25);
    ...
    bw = im2bw(P2, graythresh(P2)-0.05); %im2bw(P2,graythresh(P2)-thresh);
    bw = bwareaopen(bw,30);
...
save(sprintf('%ssegmentation_lineage_nc13.mat',export2),'centroids_x','centroids_y','convex_image',...
    'images_ms2','images_seg','images_nuc','nuc_lineage','AA','lineage_cx','lineage_cy','T');
